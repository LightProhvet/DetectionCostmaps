/*********************************************************************
Based on Navigation2 Static Layer:
https://github.com/open-navigation/navigation2/blob/main/nav2_costmap_2d/plugins/static_layer.cpp

There may be many changes still requried.

Current changes:
* rename param occurances map_topic_ and map_topic  --> semantic_map_topic_ and semantic_map_topic;
* rename namespace NB! this may be a mistake?
* rename StaticLayer -> SemanticLayer and similar occurences everywhere

 *********************************************************************/

#include "semantic_costmap/semantic_layer.hpp"
...
PLUGINLIB_EXPORT_CLASS(nav2_costmap_2d::SemanticLayer, nav2_costmap_2d::Layer)
...
SemanticLayer::onInitialize()
{
  ...
  map_sub_ = node->create_subscription<nav_msgs::msg::OccupancyGrid>(
    ...
    std::bind(&SemanticLayer::incomingMap, this, std::placeholders::_1));

  if (subscribe_to_updates_) {
    RCLCPP_INFO(logger_, "Subscribing to updates");
    map_update_sub_ = node->create_subscription<map_msgs::msg::OccupancyGridUpdate>(
      semantic_map_topic_ + "_updates",
      rclcpp::SystemDefaultsQoS(),
      std::bind(&SemanticLayer::incomingUpdate, this, std::placeholders::_1));
  }
}

void
SemanticLayer::activate()
{
}

void
SemanticLayer::deactivate()
{
...
SemanticLayer::getParameters()
{
  ...
  declareParameter("semantic_map_topic", rclcpp::ParameterValue("")); // default to costmap_publisher?
  ...
  node->get_parameter(name_ + "." + "semantic_map_topic", private_map_topic);
  node->get_parameter("semantic_map_topic", global_map_topic);
  if (!private_map_topic.empty()) {
    semantic_map_topic_ = private_map_topic;
  } else {
    semantic_map_topic_ = global_map_topic;
  }
  node->get_parameter(
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &SemanticLayer::dynamicParametersCallback,
      this, std::placeholders::_1));
}

void
SemanticLayer::processMap(const nav_msgs::msg::OccupancyGrid & new_map)
{
  RCLCPP_DEBUG(logger_, "SemanticLayer: Process map");
  ...
  RCLCPP_DEBUG(
    logger_,
    "SemanticLayer: Received a %d X %d map at %f m/pix", size_x, size_y,
    ...
    !layered_costmap_->isSizeLocked()))
  {
    ...
    RCLCPP_INFO(
      logger_,
      "SemanticLayer: Resizing costmap to %d X %d at %f m/pix", size_x, size_y,
      new_map.info.resolution);
    ...
    origin_y_ != new_map.info.origin.position.y)
  {
    ...
    RCLCPP_INFO(
      logger_,
      "SemanticLayer: Resizing semantic layer to %d X %d at %f m/pix", size_x, size_y,
      new_map.info.resolution);
    resizeMap(
      size_x, size_y, new_map.info.resolution,
...
{
  // If we are using rolling costmap, the semantic map size is
...
unsigned char
SemanticLayer::interpretValue(unsigned char value)
{
...
void
SemanticLayer::incomingMap(const nav_msgs::msg::OccupancyGrid::SharedPtr new_map)
{
  if (!map_received_) {
    processMap(*new_map);
    map_received_ = true;
...
void
SemanticLayer::incomingUpdate(map_msgs::msg::OccupancyGridUpdate::ConstSharedPtr update)
{
  std::lock_guard<Costmap2D::mutex_t> guard(*getMutex());
    ...
    x_ + width_ < update->x + update->width)
  {
    RCLCPP_WARN(
      logger_,
      "SemanticLayer: Map update ignored. Exceeds bounds of semantic layer.\n"
      "Semantic layer origin: %d, %d   bounds: %d X %d\n"
  ...
  if (update->header.frame_id != map_frame_) {
    RCLCPP_WARN(
      logger_,
      "SemanticLayer: Map update ignored. Current map is in frame %s "
  ...
  int min_i, int min_j, int max_i, int max_j)
{
  ...
  if (!map_received_in_update_bounds_) {
    ...
    if (++count == 10) {
      RCLCPP_WARN(logger_, "Can't update semantic costmap layer, no map received");
      count = 0;
    }
    return;
  }
  ...
  if (!layered_costmap_->isRolling()) {
    ...
    try {
      ...
      RCLCPP_ERROR(logger_, "SemanticLayer: %s", ex.what());
      return;
    }
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    if (param_name == name_ + "." + "map_subscribe_transient_local" ||
      param_name == name_ + "." + "semantic_map_topic" ||
